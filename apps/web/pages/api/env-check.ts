...
export default function handler(_req: NextApiRequest, res: NextApiResponse<EnvCheckResult>) {
  const result: EnvCheckResult = {
    public: {
      NEXT_PUBLIC_API_URL: typeof process.env.NEXT_PUBLIC_API_URL === 'string' ? process.env.NEXT_PUBLIC_API_URL : undefined,
      NEXT_PUBLIC_SUPABASE_URL: typeof process.env.NEXT_PUBLIC_SUPABASE_URL === 'string' ? process.env.NEXT_PUBLIC_SUPABASE_URL : undefined,
      NEXT_PUBLIC_SUPABASE_ANON_KEY_PRESENT: !!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY,
    },
    server: {
      JWT_SECRET_PRESENT: !!process.env.JWT_SECRET,
      POSTGRES_URL_PRESENT: !!process.env.POSTGRES_URL || (!!process.env.POSTGRES_HOST && !!process.env.POSTGRES_DB && !!process.env.POSTGRES_USER),
